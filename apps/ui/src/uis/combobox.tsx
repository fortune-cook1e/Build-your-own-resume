      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            variant="outline"
            aria-expanded={open}
            className="ui-w-full ui-justify-between ui-hover:bg-secondary/20 ui-active:scale-100"
          >
            <span className="ui-line-clamp-1 ui-text-left ui-font-normal">
              {props.multiple && props.value && props.value.length > 0 && (
                <span className="ui-mr-2">{props.value.join(', ')}</span>
              )}
            ...
            <CaretDown
              className={cn(
                'ui-ml-2 ui-size-4 ui-shrink-0 ui-rotate-0 ui-opacity-50 ui-transition-transform',
                open && 'ui-rotate-180',
              )}
            />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="ui-p-0">
          <Command>
            <CommandInput
              ref={ref}
              ...
              <CommandGroup>
                {props.options.map((option) => (
                  ...
                  >
                    <Check
                      className={cn(
                        'ui-mr-2 ui-size-4 ui-opacity-0',
                        !props.multiple &&
